...
import {
  compileAll, computeView, segments, scanMatches, deriveFields,
} from "../src/logic.ts";
...
const TAGS = ["wifi", "ble", "pmu", "sensor", "ota", "fs", "rtc", "uart", "i2c", "cpu"];
const MSGS = [
...
function genLog(n: number, seed: number): string[] {
  ...
  for (let i = 0; i < n; i++) {
    ...
    lines[i] = `${hh}:${mm}:${ss}.${mmm} [${pick(LEVELS)}] ${pick(TAGS)}: ${body}`;
  }
  return lines;
}
...
function genFilters(n: number): Filter[] {
  ...
  const fillers = ["ble", "pmu", "sensor", "ota", "fs", "rtc", "uart", "i2c",
    "cpu", "heap", "reset", "battery", "packet", "calibration", "flash",
    "irq", "state", "checksum", "watchdog", "latency"];
  let i = 0;
...
interface Stat { name: string; min: number; median: number; mean: number; ops: number; }

function bench(name: string, fn: () => void, runs = RUNS, warmup = WARMUP): Stat {
...
if (!JSON_OUT) {
  console.log(`Logsy profiler — ${LINES.toLocaleString()} lines, ${FILTERS} filters, ${RUNS} runs (seed ${SEED})\n`);
...
if (!JSON_OUT) console.log(`${ms(performance.now() - genStart)}  (${(bytes / 1e6).toFixed(1)} MB)\n`);

// Sample line + filter shape (sanity check the synthetic data is realistic).
if (!JSON_OUT) {
  console.log(`  e.g. ${lines[0]}`);
  console.log(`  filters: ${filters.length} (${filters.filter((f) => f.regex).length} regex, ${filters.filter((f) => f.exclude).length} exclude)\n`);
}
...
stats.push(bench("compileAll", () => { compileAll(filters); }));

// 2. computeView — THE hot path: every line tested against every usable filter.
stats.push(bench("computeView (full file)", () => { computeView(lines, compiled); }));

// 3. fieldsFor — lazy field extraction; cost if every visible row is expanded.
stats.push(bench("fieldsFor × all rows", () => {
...
const winRe = compiled.find((c) => c.re && c.f.pattern === "wifi")?.re ?? compiled[0].re!;
const window = lines.slice(0, Math.min(1000, lines.length));
stats.push(bench("segments × 1000 rows", () => {
...
stats.push(bench("scanMatches (preview)", () => { scanMatches(lines, previewRe); }));

// --- report -----------------------------------------------------------------

if (JSON_OUT) {
  console.log(JSON.stringify({
    config: { lines: LINES, filters: FILTERS, runs: RUNS, seed: SEED, bytes },
    stats,
  }, null, 2));
} else {
  const pad = (s: string, w: number) => s.padEnd(w);
  const padN = (s: string, w: number) => s.padStart(w);
  console.log(pad("benchmark", 26) + padN("min", 11) + padN("median", 11) + padN("mean", 11) + padN("ops/s", 9));
  console.log("─".repeat(68));
  for (const s of stats) {
    console.log(
      pad(s.name, 26) +
      padN(ms(s.min), 11) + padN(ms(s.median), 11) + padN(ms(s.mean), 11) +
