...
const viewOf = (lines: string[], filters: Filter[]) => computeView(lines, compileAll(filters));
const track = (filterId: string, timeField: string, over: Partial<TimelineSource> = {}): TimelineSource =>
  ({ id: "t_" + filterId + "_" + timeField, filterId, timeField, lane: timeField, kind: "point", unit: "hms", ...over });
...
test("trackFieldsOf returns every field — any field can back a track, not just numeric", () => {
  const f = filter("a", "(?<ts>x)(?<lvl>y)(?<n>z)(?<h>w)", [
    { name: "ts", type: "time" }, { name: "lvl", type: "string" },
    { name: "n", type: "int" }, { name: "h", type: "hex" },
  ]);
...
test("only added lines produce events", () => {
  ...
  const v = viewOf(lines, [filter("f", "(?<ts>\\d+:\\d+:\\d+\\.\\d+)\\s+(?<msg>.*)",
    [{ name: "ts", type: "time" }, { name: "msg", type: "string" }])]);
...
test("first-match precedence: the field provider is the first matching filter", () => {
  ...
  const v = viewOf(lines, [
    filter("hi", "(?<ts>\\d+:\\d+:\\d+\\.\\d+)", [{ name: "ts", type: "time" }]),
    filter("lo", "(?<ts>\\d+:\\d+:\\d+\\.\\d+)", [{ name: "ts", type: "time" }]),
  ]);
...
test("one filter with several time fields yields one event per track", () => {
  ...
  const v = viewOf(lines, [filter("f", "send=(?<s>\\d+)\\s+recv=(?<r>\\d+)",
    [{ name: "s", type: "time" }, { name: "r", type: "time" }])]);
  const marks = buildTimeline(v, [1], [track("f", "s", { unit: "ms" }), track("f", "r", { unit: "ms" })]);
  expect(marks.map((m) => [m.lane, m.t])).toEqual([["s", 100_000_000], ["r", 180_000_000]]);
});

test("a non-time numeric field can be a timestamp with a declared unit", () => {
  const lines = ["seq=4200"];
  const v = viewOf(lines, [filter("f", "seq=(?<seq>\\d+)", [{ name: "seq", type: "int" }])]);
...
test("a span track pairs start/end fields from the same line", () => {
  ...
  const marks = buildTimeline(v, [1], [track("f", "a", { kind: "span", endField: "b", unit: "ms" })]);
...
test("hidden tracks emit nothing; the mark carries the line's parsed fields", () => {
  ...
  const v = viewOf(lines, [filter("f", "(?<ts>\\d+:\\d+:\\d+\\.\\d+)\\s+(?<tag>\\w+)",
    [{ name: "ts", type: "time" }, { name: "tag", type: "string" }])]);
