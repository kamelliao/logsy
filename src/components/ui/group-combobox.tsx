...
export function GroupCombobox({ value, groups, onChange }: GroupComboboxProps) {
  ...
  const labelOf = (id: string) =>
    id === NONE ? "No group (ungrouped)" : (byId.get(id)?.name ?? id);
  ...
  return (
    <Combobox.Root
      ...
      value={current}
      onValueChange={(v) => {
        if (typeof v === "string") onChange(v === NONE ? null : v);
      }}
      itemToStringLabel={labelOf}
    >
      <Combobox.Trigger className="section-select">
        <span className={"ss-label" + (value === null ? " placeholder" : "")}>
          {labelOf(current)}
        </span>
        <Combobox.Icon className="ss-chev">
          <ChevronDown size={15} />
        </Combobox.Icon>
      </Combobox.Trigger>
      <Combobox.Portal>
