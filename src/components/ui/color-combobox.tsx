...
export function ColorCombobox({
  ...
  const byColor = new Map(
    options.map((o) => [o.color.toLowerCase(), o] as const),
  );
  ...
  const swatch = (color: string) =>
    kind === "text" ? (
      <span className="cc-swatch cc-swatch-text" style={{ color }}>
        A
      </span>
    ) : (
  ...
  return (
    ...
    >
      <Combobox.Trigger className="cc-trigger">
        ...
        <Combobox.Icon className="cc-chev">
        ...
        >
          <Combobox.Popup className="menu-pop cc-popup">
            <div className="cc-search">
              <Search size={14} />
              <Combobox.Input
                placeholder={placeholder ?? "Search color…"}
                className="cc-input"
              />
            </div>
            ...
            <Combobox.List className="cc-list">
              {(color: string) => (
                <Combobox.Item key={color} value={color} className="cc-item">
                  ...
                  <Combobox.ItemIndicator className="cc-check">
                    <Check size={14} />
