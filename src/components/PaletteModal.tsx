...
let _counter = 0;
function mkId() { return `pal_${_counter++}`; }
...
function autoName(text: string, bg: string): string {
  const t = TEXT_SWATCHES.find((s) => s.color.toLowerCase() === text.toLowerCase())?.name;
  const b = BG_SWATCHES.find((s) => s.color.toLowerCase() === bg.toLowerCase())?.name;
  if (t && b) return `${t}/${b}`;
  return b ?? t ?? bg;
}
...
function ColorPairPicker({ text, bg, onTextChange, onBgChange }: ColorPairPickerProps) {
  ...
  useEffect(() => { setHexText(text); }, [text]);
  useEffect(() => { setHexBg(bg); }, [bg]);
    ...
    <>
      ...
      {open && pos && createPortal(
        <div
          ...
          style={{ position: "fixed", bottom: pos.bottom, left: pos.left, zIndex: 200 }}
        >
          ...
          <div className="csp-swatches">
            {TEXT_SWATCHES.map((opt) => (
              <button
                key={opt.color}
                type="button"
                className={"csp-swatch" + (opt.color.toLowerCase() === text.toLowerCase() ? " active" : "")}
                style={{ background: opt.color }}
                title={opt.name}
                onClick={() => { onTextChange(opt.color); setHexText(opt.color); }}
              />
            ))}
          </div>
          <div className="csp-custom">
            <input
              ...
              onChange={(e) => { setHexText(e.target.value); onTextChange(e.target.value); }}
            />
            <input
              type="text"
              className="csp-hex"
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") { commitHex(hexText, "text"); (e.target as HTMLInputElement).blur(); }
          ...
          <div className="csp-swatches">
            {BG_SWATCHES.map((opt) => (
              <button
                key={opt.color}
                type="button"
                className={"csp-swatch" + (opt.color.toLowerCase() === bg.toLowerCase() ? " active" : "")}
                style={{ background: opt.color }}
                title={opt.name}
                onClick={() => { onBgChange(opt.color); setHexBg(opt.color); }}
              />
            ))}
          </div>
          <div className="csp-custom">
            <input
              ...
              onChange={(e) => { setHexBg(e.target.value); onBgChange(e.target.value); }}
            />
            <input
              type="text"
              className="csp-hex"
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") { commitHex(hexBg, "bg"); (e.target as HTMLInputElement).blur(); }
                if (e.key === "Escape") setOpen(false);
              }}
            />
          </div>

          <div className="cpp-preview" style={{ background: bg, color: text }}>
            Sample Text
          </div>
        </div>,
        document.body
...
function SortablePaletteRow({ row, onRename, onDelete }: {
  row: PaletteRow;
  ...
  function commit() {
    const v = val.trim();
    if (v) onRename(row._id, v); else setVal(row.name);
    setEditing(false);
  }
  ...
  return (
    ...
    >
      ...
      <span className="pm-swatch" style={{ background: row.bg, color: row.text }}>A</span>
      {editing ? (
        <input
        ...
        <span
          className="pm-name"
          onDoubleClick={() => { setVal(row.name); setEditing(true); }}
      ...
      <button type="button" className="pm-delete" onClick={() => onDelete(row._id)} title="Remove">
        <X size={13} />
      </button>
    </div>
  );
...
export function PaletteModal({ palette, onChange, onClose }: PaletteModalProps) {
  ...
  const [resetPos, setResetPos] = useState<{ bottom: number; left: number } | null>(null);
  ...
  function openReset() {
    ...
    if (rect) setResetPos({ bottom: window.innerHeight - rect.top + 6, left: rect.left });
    setResetOpen(true);
  }
  ...
  const addAlreadyExists = rows.some(
    (r) => r.text.toLowerCase() === addText.toLowerCase() && r.bg.toLowerCase() === addBg.toLowerCase()
  );

  function handleAdd() {
    if (addAlreadyExists) return;
    apply([...rows, { name: resolvedName, text: addText, bg: addBg, _id: mkId() }]);
    setAddName("");
  ...
  return (
    <>
      <Dialog open onOpenChange={(o) => { if (!o) onClose(); }}>
        <DialogContent style={{ width: 520 }}>
          <DialogHeader>
            ...
            <Button variant="ghost" size="icon" className="mh-x" onClick={onClose}>
              <X size={18} />
            </Button>
          </DialogHeader>
          ...
          <div className="modal-body pm-body">
            <p className="pm-desc">
              Customise the preset swatches shown in the Color row when editing a filter.
      ...
      {resetOpen && resetPos && createPortal(
        <div
          ref={resetPanelRef}
          className="pm-reset-pop menu-pop"
          style={{ position: "fixed", bottom: resetPos.bottom, left: resetPos.left, zIndex: 200 }}
        >
          <p className="pm-reset-msg">Reset to the default palette? All custom entries will be removed.</p>
          <div className="pm-reset-actions">
            <Button variant="ghost" size="sm" onClick={() => setResetOpen(false)}>Cancel</Button>
            <Button size="sm" onClick={() => { handleReset(); setResetOpen(false); }}>Reset</Button>
          </div>
        </div>,
        document.body
      )}
