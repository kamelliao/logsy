...
export function FilterPanel({
  ...
  // The filter-set LIST is global (one ordered list shared by every file); the
  // active SELECTION is per-document, on this file (`file.activeSetId`), resolved to
  // the first set when unset.
  const sets = useStore((s) => s.doc.filterSets);
  const activeSetId = file.activeSetId ?? sets[0]?.id ?? null;
