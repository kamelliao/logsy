...
/**
 * The palette editor body — the sortable preset list plus the add-new row —
 * with no dialog chrome, so it can live inside the Settings dialog's Colors
 * page. Keeps its own working copy of the rows (seeded from `palette`) and
 * pushes every edit up through `onChange`.
 */
export function PaletteEditor({ palette, onChange }: PaletteEditorProps) {
  ...
  function handleReset() {
    setRows(toRows(DEFAULT_PALETTE));
  ...
  return (
    <div className="pm-editor">
      <p className="pm-desc">
        Customise the preset swatches shown in the Color row when editing a
        filter. Double-click a name to rename it.
      </p>
      ...
      {/* ── add section (reset lives on this header row to save vertical space) ── */}
      <div className="pm-add-head">
        <div className="pm-add-title">New preset</div>
        <div className="pm-reset-inline">
          {confirmingReset ? (
            <>
              <span className="pm-reset-msg">Reset palette?</span>
              <Button
                variant="ghost"
                size="sm"
